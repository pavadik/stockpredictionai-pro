...
def _build_params(json_dict: dict, capital: float, point_mult: float,
                  direction: str, args=None) -> StrategyParams:
    ...
    if args:
        d["adx_threshold"] = args.adx_threshold
        d["adx_action"] = args.adx_action
...
def main():
    ...
    ap.add_argument("--tp_atr_mult", type=float, default=0.0,
                    help="Take-profit target in ATR multiplier (0.0 = disabled)")
    ap.add_argument("--tp_pct", type=float, default=0.5,
                    help="Fraction of position to close on take-profit (e.g., 0.5 = 50%)")
    ap.add_argument("--adx_threshold", type=float, default=0.0,
                    help="Minimum D1 ADX value to trade full size (0.0 = disabled)")
    ap.add_argument("--adx_action", default="skip",
                    choices=["skip", "half"],
                    help="Action when ADX < threshold: skip (no entry) or half (reduce size)")
    args = ap.parse_args()
    ...
    with open(os.path.join(OUT_DIR, "params", "block_e_best_params.json")) as f:
        long_json = json.load(f)
    with open(os.path.join(OUT_DIR, "params", "block_e_short_best_params.json")) as f:
        short_json = json.load(f)

    params_L = _build_params(long_json, capital, args.point_mult, "long", args)
    params_S = _build_params(short_json, capital, args.point_mult, "short", args)
    ...
    tp_label = f"tp_atr={args.tp_atr_mult:.1f}, tp_pct={args.tp_pct*100:.0f}%" if args.tp_atr_mult > 0 else "disabled"
    print(f"  Flip mode: {args.flip_mode} | Take-profit: {tp_label}")
    ...
    report_lines = [
        ...
        "",
        "  --- LONG Parameters ---",
        f"  tf1={long_json['tf1']}  tf2={long_json['tf2']}  "
        ...
        "",
        "  --- SHORT Parameters ---",
        f"  tf1={short_json['tf1']}  tf2={short_json['tf2']}  "
