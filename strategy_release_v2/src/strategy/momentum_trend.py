...
class StrategyParams:
    ...
    # --- Regime filter (ADX D1) -------------------------------------------
    adx_period: int = 14
    """Period for daily ADX calculation."""

    adx_threshold: float = 0.0
    """Minimum ADX value to trade full size (0.0 = disabled)."""

    adx_action: str = "skip"
    """Action when ADX < threshold: ``skip`` (no entry) or ``half`` (reduce size by 50%)."""
...
def prepare_strategy_data(df_m1: pd.DataFrame,
    ...
    # 1. Aggregate to daily bars
    ohlcv_agg = {
        "open": "first", "high": "max", "low": "min", "close": "last"
    }
    df_d1 = df_m1.groupby(df_m1.index.date).agg(ohlcv_agg)
    # 2. Calculate ADX
    adx_d1 = adx(df_d1, window=params.adx_period)
    # 3. Shift by 1 day to prevent lookahead bias (use yesterday's ADX for today's trading)
    adx_d1_shifted = adx_d1.shift(1)
    # 4. Map to intraday dataframe
    df["date_only"] = df.index.date
    df["adx_d1"] = df["date_only"].map(adx_d1_shifted).fillna(0.0)
...
def generate_signals(df: pd.DataFrame,
    ...
    if params.adx_threshold > 0 and "adx_d1" in df.columns:
        if params.adx_action == "skip":
            df["entry_long"] = df["entry_long"] & (df["adx_d1"] >= params.adx_threshold)
        elif params.adx_action == "half":
            raw_contracts = np.where(df["adx_d1"] < params.adx_threshold, raw_contracts / 2, raw_contracts)

    df["contracts"] = pd.Series(raw_contracts, index=df.index).fillna(0).astype(int).clip(lower=0)
