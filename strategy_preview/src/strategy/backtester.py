    ...
    for ts, src, idx in events:
        ...
        if allow_val and in_win_val:
            if pos_size == 0:
                ...
                if entry_flag:
                    if sdel_day == 1:
                        ...
                        if src == "short" and one_entry_today_S:
                            continue
                    atr_for_entry = float(M["atr"][idx]) if "atr" in M else 0.0
                    _open_position(src, ts, close_val, lots_val, atr_for_entry)
                    if src == "long":
                ...
                # same direction bar -> check signal_reverse and take-profit
                if not entry_flag:
                    _close_position(ts, close_val, "signal_reverse")
                else:
                    # check take-profit
                    if tp_atr_mult > 0 and tp_pct > 0 and not tp_hit and entry_atr > 0:
                        pnl_sign = 1.0 if pos_dir == "long" else -1.0
                        unrealized = pnl_sign * (close_val - entry_price)
                        tp_target = entry_atr * tp_atr_mult
